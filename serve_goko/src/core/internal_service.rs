...
impl<T: Send, S: Send> Message<T, S> {
    ...
    pub(crate) fn respond(&mut self, response: Result<S, GokoError>) {
        match self.reply.take() {
            Some(reply) => match reply.send(response.map_err(|e| InternalServiceError::from(e))) {
                Ok(_) => (),
                Err(_) => {
                    *self.global_error.lock().unwrap() =
                        Some(Box::new(InternalServiceError::FailedRespSend));
                }
            },
            None => {
                *self.global_error.lock().unwrap() =
                    Some(Box::new(InternalServiceError::DoubleRead))
            }
        }
    }
...
impl<T: Send + 'static, S: Send + 'static> InternalServiceOperator<T, S> {
    pub(crate) fn new<P: InternalService<T, S> + 'static>(
        mut server: P,
    ) -> InternalServiceOperator<T, S> {
    ...
    pub(crate) fn message(&self, request: T) -> ResponseFuture<S> {
        ...
        let (reply, response): (CoreResponseSender<S>, CoreResponseReciever<S>) =
            oneshot::channel();
        ...
        let error = self
            ...
            .send(msg)
            .err()
            .map(|_e| InternalServiceError::FailedSend);
        ResponseFuture {
            response,
            flight_counter,
            error,

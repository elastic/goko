...
impl BucketHKLDivergence {
    ...
    fn add_trace_to_pdfs(&mut self, trace:&[NodeAddress]) {
        ...
        for (parent,child) in parent_address_iter.zip(child_address_iter) {
            self.running_pdfs.entry(*parent).or_insert(BucketProbs::new()).add_child_pop(Some(*child),1);
        }
        let last = trace.last().unwrap();
        self.running_pdfs.entry(*last).or_insert(BucketProbs::new()).add_child_pop(None,1);
    }
    ...
    fn remove_trace_from_pdfs(&mut self, trace:&[NodeAddress]) {
        ...
        child_address_iter.next();
        for (parent,child) in parent_address_iter.zip(child_address_iter) {
            self.running_pdfs.entry(*parent).or_insert(BucketProbs::new()).remove_child_pop(Some(*child),1);
        }
        let last = trace.last().unwrap();
        self.running_pdfs.entry(*last).or_insert(BucketProbs::new()).remove_child_pop(None,1);
    }
    ...
    pub fn max_kl<M:Metric>(&self,tree_reader:&CoverTreeReader<M>) -> Option<(f32,NodeAddress)> {
        let mut max_kl: Option<(f32,NodeAddress)> = None;
        for (address,sequence_pdf) in self.running_pdfs.iter() {
            let kl = tree_reader.get_node_plugin_and::<BucketProbs,_,_>(*address,|p| sequence_pdf.kl_divergence(p)).unwrap(); 
            if let Some(kl) = kl {

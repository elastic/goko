...
impl DiagGaussianNode {
    ...
    pub fn var(&self) -> Vec<f32> {
        self.mom2.iter().map(|x| x/(self.count as f32)).collect()
    }
}
...
impl<M: Metric> GrandmaPlugin<M> for GrandmaDiagGaussian {
    ...
    fn node_component(
        ...
        let mut mom1 = my_tree
            .parameters()
            .point_cloud
            .moment_subset(1, my_node.singletons()).unwrap();
        let mut mom2 = my_tree
            .parameters()
            .point_cloud
            .moment_subset(2, my_node.singletons()).unwrap();
        ...
        if let Some((nested_scale, child_addresses)) = my_node.children() {
            my_tree.get_node_plugin_and::<Self::NodeComponent, _, _>((nested_scale,*my_node.center_index()), |p| {
            ...
            let my_center = my_tree.parameters().point_cloud.get_point(*my_node.center_index()).unwrap();
...
pub(crate) mod tests {
    ...
    fn gaussian_sanity_correct() {
            ...
            layer.for_each_node(|pi, n| {
                if n.is_leaf() {
                    n.get_plugin_and::<DiagGaussianNode, _, _>(|p| {
                        println!("=====<Leaf ({},{})>=====",n.scale_index(),n.center_index());
                        println!("DiagGauss: {:?}, Singles: {:?}, Center Index: {:?}", p, n.singletons(), n.center_index());
                        let singles = n.singletons();
                        let mut s_mom1 = singles.iter().map(|i| basic_tree_data[*i as usize]).fold(0.0,|a,x| a+x);
                        let mut s_mom2 = singles.iter().map(|i| basic_tree_data[*i as usize]).fold(0.0,|a,x| a+x*x);
                        s_mom1 += basic_tree_data[*n.center_index() as usize];
                        s_mom2 += basic_tree_data[*n.center_index() as usize] *basic_tree_data[*n.center_index() as usize];
                        let s_count = singles.len() + 1;
                        println!("First moment, expected: {:?}, calculated: {:?}", s_mom1, p.mom1[0]);
                        assert_approx_eq!(s_mom1, p.mom1[0]);
                        println!("Second moment, expected: {:?}, calculated: {:?}", s_mom2, p.mom2[0]);
                    ...
                    n.get_plugin_and::<DiagGaussianNode, _, _>(|dp| {
                        println!("=====<Routing ({},{})>=====",n.scale_index(),n.center_index());
        ...
        reader.get_node_plugin_and::<DiagGaussianNode,_,_>(reader.root_address(),|p| {
            println!("First moment, expected: {:?}, calculated: {:?}", mom1, p.mom1[0]);
            assert_approx_eq!(mom1, p.mom1[0]);
            println!("Second moment, expected: {:?}, calculated: {:?}", mom2, p.mom2[0]);

...
impl CoveredData {
    pub(crate) fn new<D: PointCloud>(point_cloud: &Arc<D>) -> GokoResult<CoveredData> {
    ...
    pub(crate) fn split(self, thresh: f32) -> GokoResult<(CoveredData, UncoveredData)> {
        let mut close_index = Vec::with_capacity(self.coverage.len());
        let mut close_dist = Vec::with_capacity(self.coverage.len());
        let mut far = Vec::new();
